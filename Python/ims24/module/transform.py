...
class Transform(object):
    ...
    def cleasing_1_addresses():
        ...
        df[['street_clean','house_number']] = pd.DataFrame(df['street'].str.split(' ',1).tolist(),columns = ['street',
                                                                                                             'row'])
    @staticmethod
    ...
    def soft_delete_raw_data():
        engine.execute(
            text("update ods.extracted_raw_table set deleted = true where deleted = false;").execution_options(
                autocommit=True))
