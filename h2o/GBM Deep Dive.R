...
#Gradient Boosting Machine é um meta-algoritmo de apredizado supervisionado que é geralmente utilizado em 
#problemas de classificação e regressão. O principio algortítmico por trás do GBM é a produção de previsões 
#derivadas de modelos preditivos fracos (Weak Learners), em especial árvores de decisão em que essas árvores são 
#combinadas via ensemble para redução de viéses dos algoritmos. Essas previsões são geradsas através da combinação da 
#metaheurística de gradiente descendente para otimização paramétrica face uma função de custo que é minimizada, 
#e do Boosting que é combinação de diversos classificadores fracos (Weak Learners) em série para (ou meta classificador) 
#para combinação de resultados desses algoritmos. 

#Como lida com um conjunto bem robusto de weak learners é insensível a distribuições de cauda longa e 
#outliers além de responder bem à não linearidade.
